...
import { Search, Heart, User, ShoppingCart, Menu, X, Bed, Home, HelpCircle, MapPin, ShieldCheck, Phone, MessageSquare, LogOut, Shield, LayoutGrid } from 'lucide-react';
...
export default function Header({
  ...
  const handleNavClick = (catId: string) => {
    setCategory(catId);
    setSearchQuery(''); // Always reset search query when clicking navigation links or Home
    setMobileMenuOpen(false);
    
    if (catId === 'all') {
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } else {
      const gridElement = document.getElementById('product-catalog-section') || document.getElementById('product-catalog');
      if (gridElement) {
    ...
    <header className="bg-white sticky top-0 z-50 border-b border-border-light shadow-sm transition-all" dir="rtl">
      ...
      <div className="bg-[#004D95] text-white py-1.5 px-4 text-xs sm:text-sm font-bold flex justify-between items-center gap-2 border-b border-[#003C75]">
        ...
        <div className="hidden sm:flex items-center gap-4">
          ...
          <a href="https://wa.me/201034462884" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-[#49B2A4] transition-colors bg-white/10 px-2 py-0.5 rounded">
            <MessageSquare className="w-3.5 h-3.5 text-[#49B2A4]" />
            <span>واتساب سريع: 01034462884</span>
          </a>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 md:h-20 flex justify-between items-center">
        ...
        <nav className="hidden lg:flex items-center gap-8 font-medium">
          <button
            onClick={() => handleNavClick('all')}
            className={`text-sm py-1 transition-colors cursor-pointer flex items-center gap-1.5 ${
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-border-light h-16 z-[45] flex items-center justify-around px-4 shadow-xl">
        ...
        <button
          onClick={() => handleNavClick('mattresses')}
          className={`flex flex-col items-center justify-center flex-1 cursor-pointer ${
            currentCategory === 'mattresses' ? 'text-primary font-bold' : 'text-secondary hover:text-primary'
          }`}
        >
          <Bed className="w-5 h-5" />
          <span className="text-[10px] mt-1 font-medium">المراتب</span>
        </button>
        <button
          onClick={() => {
