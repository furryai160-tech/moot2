...
import { calculateShippingCost } from '../data';
...
export default function CartDrawer({
  ...
  const enableFreeThreshold = shippingConfig?.enableFreeShippingThreshold ?? true;
  const shippingThreshold = enableFreeThreshold ? (shippingConfig?.freeShippingThreshold ?? 2000) : 99999999;
  const isFreeShipping = enableFreeThreshold && subtotal >= shippingThreshold;
  const shippingCost = subtotal === 0 ? 0 : calculateShippingCost(shippingConfig, 'القاهرة', subtotal);
          ...
          <div className="border-t border-border-light p-4 sm:p-6 bg-gray-50 flex-shrink-0 text-right space-y-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <span>إتمام طلب الشراء الآن</span>
              </button>
              
              <button
                onClick={onClose}
                className="w-full bg-white hover:bg-gray-100 text-charcoal border border-border-light py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer shadow-sm"
              >
                <span>إكمال التسوق</span>
              </button>
            </div>
