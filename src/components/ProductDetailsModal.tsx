...
export default function ProductDetailsModal({
  ...
  // Compute available sizes or default mattress sizes if applicable
  const availableSizes = (product.sizes && product.sizes.length > 0)
    ? product.sizes
    : (product.category === 'mattresses' || product.name.includes('مرتبة') || product.name.includes('ملاية'))
      ? [
          { size: '100×195 سم', price: product.price },
          { size: '120×195 سم', price: product.price },
          { size: '140×195 سم', price: product.price },
          { size: '160×195 سم', price: Math.round(product.price * 1.15) },
          { size: '180×195 سم', price: Math.round(product.price * 1.25) },
          { size: '200×200 سم', price: Math.round(product.price * 1.35) },
        ]
      : [];

  const [selectedSize, setSelectedSize] = useState<string | undefined>(() => {
    if (initialSize) return initialSize;
    return availableSizes.length > 0 ? availableSizes[0].size : undefined;
  });

  // Custom size states
  const [isCustomSize, setIsCustomSize] = useState<boolean>(false);
  const [customWidth, setCustomWidth] = useState<string>('170');
  const [customLength, setCustomLength] = useState<string>('198');
  const [customHeight, setCustomHeight] = useState<string>('26');
  ...
  const activePrice = (selectedSize && availableSizes.length > 0)
    ? (availableSizes.find(s => s.size === selectedSize)?.price ?? product.price)
    : product.price;

  const activeOldPrice = (selectedSize && availableSizes.length > 0)
    ? (availableSizes.find(s => s.size === selectedSize)?.oldPrice ?? product.oldPrice)
    : product.oldPrice;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-right">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 bg-[#fbfbfb] p-3 rounded-xl border border-border-light">
                ...
                {product.category === 'mattresses' || (product.name || '').includes('مرتبة') || (product.name || '').includes('مرتبه') ? (
                  <div className="flex items-center gap-2 justify-end text-xs text-[#004D95] font-bold">
                    <span>ضمان ١٠ سنوات معتمد</span>
                    <Shield className="w-4 h-4 text-[#004D95]" />
                  </div>
                ) : (
                  <div className="flex items-center gap-2 justify-end text-xs text-muted-gray font-medium">
                    <span>الضمان: للمراتب الطبية فقط</span>
            ...
            <div className="flex flex-col justify-between">
              <div>
                ...
                {availableSizes.length > 0 && (
                  <div className="mb-6 bg-gray-50/70 p-3.5 rounded-2xl border border-gray-200 space-y-3">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {availableSizes.map((szObj) => {
                        ...
                        return (
                          ...
                          >
                            <div>{szObj.size}</div>
                            <div className={`text-[10px] mt-0.5 ${isSel ? 'text-white/90' : 'text-primary'}`}>
